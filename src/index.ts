...
setTimeout(() => {
  connectHomeAssistant(HA_URL, HA_TOKEN, devices, log).catch((e: unknown) =>
    log('Home Assistant startup error:', e)
  );
}, 1000);
...
async function changeState({ name, value }: { name: string; value: Device['state'] }) {
  ...
  if (device.entity_id && haConnection) {
    ...
    log(
      `HA callService: ${callDomain}.${service} for ${device.entity_id} value=${value}: ${JSON.stringify(res)}`
    );
    return { ok: true, res };
  }
  ...
  execute: ({ message }: { message: string }) =>
    withToolErrorHandling('smarthome_tts_voice', async () => {
      if (client.connected) {
        ...
        return {
          content: [
            { type: 'text', text: `MQTT client not connected. Could not speak: ${message}` }
          ]
        };
      }
    })
...
if (process.argv[1] && path.resolve(process.cwd(), process.argv[1]) === path.resolve(__filename)) {
